...
set ROOT=%~dp0..
for %%V in (LIVEKIT_URL LIVEKIT_API_KEY LIVEKIT_API_SECRET OPENROUTER_API_KEY OPENAI_API_KEY ENABLE_VOICE VOICE_PROVIDER OPENAI_REALTIME_MODEL) do call :load_env %%V
...
start "agent-vinod-backend" /min cmd /c call "%~dp0run-backend-stagehand-cloud.cmd" ^> "%BACKEND_OUT%" 2^> "%BACKEND_ERR%"
...
:load_env
for /f "usebackq tokens=1,* delims==" %%A in (`findstr /b /c:"%~1=" "%ROOT%\.env"`) do set "%~1=%%B"
exit /b 0
